    ...
    async function fetchAdvice() {
      ...
      try {
        const response = await fetch(
          `https://deelay.me/500/https://api.adviceslip.com/advice/search/${apiStatus.query}`
        );
...
function App() {
  ...
  const debouncedHandler = debounce((e) => changeHandler(e.target.value), 800);
  return (
    <div className="App">
      <header className="App-header">
        ...
        <label htmlFor="query">
          ...
          <input name="query" onChange={debouncedHandler} />
        </label>
